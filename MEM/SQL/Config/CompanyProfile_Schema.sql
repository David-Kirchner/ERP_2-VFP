...
BEGIN
    CREATE TABLE dbo.HPAcertPdf (
        ...
        -- Input snapshot (what version of every field was used) --------
        CompanySnapshotJson  NVARCHAR(MAX) NOT NULL,     -- frozen copy of CompanyProfile row
        PlantSnapshotJson    NVARCHAR(MAX) NOT NULL,     -- frozen copy of CompanyPlant row
        SignerSnapshotJson   NVARCHAR(MAX) NOT NULL,     -- frozen array of signer rows
        RenderEngine         NVARCHAR(40)  NOT NULL,     -- 'VFP_FRX' | 'SSRS' | 'FastReport'
...
/* -----------------------------------------------------------------------------
   ...
   If HPAcertKey is the wrong type, change the column type instead of casting.
   ----------------------------------------------------------------------------- */
