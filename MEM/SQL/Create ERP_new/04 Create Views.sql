...
SELECT DISTINCT 
                         ...
                         dbo.f_Vendor_QAType_Other(v.QAType) AS Other, CASE Vend_Notes WHEN ''^*^*^'' THEN '''' ELSE Vend_Notes END AS Vend_Notes,
                             (SELECT        TOP (1) OrderDate
                               ...
                               ORDER BY POitem DESC) AS [Last Used Date], CASE ERP_2.dbo.f_IsVendorAppr(v.VendCode) WHEN ''1'' THEN ''Approved'' WHEN ''2'' THEN ''Conditional'' ELSE ''Unapproved'' END AS [Calculated Approval], 
...
FROM            (SELECT        *
                          FROM            [ERP_2].[dbo].[Survey_Assignments] SA
                          WHERE        SA.[AssignmentType] = 1) SA LEFT JOIN
                         [ERP_2].[dbo].[TC_Replacement_Employees] employee ON SA.[TargetIdentifier] = CONVERT(varchar, employee.EmployeeId)
UNION
...
FROM            (SELECT        *
                         ...
                         [ERP_2].[dbo].[AllQuotes] quote ON SA.[TargetIdentifier] = RTRIM(LTRIM(CONVERT(varchar, quote.InqNumP))) LEFT JOIN
                         [ERP_2].[dbo].[Survey_Assignment_ContactInfos] general ON SA.[TargetIdentifier] = CONVERT(varchar(50), general.ID)
UNION
...
FROM            (SELECT        *
                         ...
                         [ERP_2].[dbo].[AllQuotes] sales ON SA.[TargetIdentifier] = CONVERT(varchar, sales.SalesNum) LEFT JOIN
                         [ERP_2].[dbo].[Survey_Assignment_ContactInfos] general ON SA.[TargetIdentifier] = CONVERT(varchar(50), general.ID)
UNION
...
FROM            (SELECT        *
                         ...
                         [ERP_2].[dbo].[AllQuotes] purchaseOrder ON SA.[TargetIdentifier] = purchaseOrder.PO_num LEFT JOIN
                         [ERP_2].[dbo].[Survey_Assignment_ContactInfos] general ON SA.[TargetIdentifier] = CONVERT(varchar(50), general.ID)
UNION
...
(
	SELECT DISTINCT 
		...
		[ERP_2].[dbo].f_GetPcsShipped_SOitem(s.SOitem) AS [PiecesShipped], 
		a.pieces AS [PiecesQuoted], 
		LTRIM([ERP_2].[dbo].f_GetTopPOitem_SOitem(a.SOitem)) AS POitem, 
		[ERP_2].[dbo].f_Branch_Form(a.form) AS Branch, 
		ISNULL(s.cmplt, 0) AS Completed, 
		ISNULL(s.Hold, CONVERT(bit, 0)) AS Hold, 
		[ERP_2].[dbo].[f_Who_Held_SOitem](shs.SOitem) AS HeldBy
	FROM 
	(
		SELECT DISTINCT 
			s.soitem AS [SOitem], 
			...
			s.cmplt, 
			s.Hold
		FROM [ERP_2].[dbo].[Sales] AS s WITH (NOLOCK)
		...
		SELECT DISTINCT 
			...
			s.cmplt, 
			s.Hold
		FROM [ERP_2].[dbo].[Sales_Hold_Status] AS shs WITH (NOLOCK) 
		JOIN [ERP_2].[dbo].[Sales] AS s WITH (NOLOCK) ON shs.SOitem = s.SOitem
		WHERE [ERP_2].[dbo].[f_Is_SOitemHeld](shs.SOitem) = 1 
		AND (ISNULL(s.cncld, 0) < 1) 
		AND (ISNULL(s.itemtot, 0) > 0)
    ) AS s 
    LEFT JOIN [ERP_2].[dbo].[PackingSlip] AS ps WITH (NOLOCK) ON s.SOitem = ps.[SOitem] 
    INNER JOIN [ERP_2].[dbo].AllQuotes AS a WITH (NOLOCK) ON s.soitem = a.SOitem 
    INNER JOIN [ERP_2].[dbo].Sales_No AS sn WITH (NOLOCK) ON s.salesnum = sn.salesnum 
    LEFT JOIN [ERP_2].[dbo].[Sales_Hold_Status] AS shs WITH (NOLOCK) ON s.SOitem = shs.SOitem
) AS Q 
OUTER APPLY 
(
    SELECT 
    ...
    FROM (
        SELECT TOP (1) 
            ...
            (
                ...
                INNER JOIN [ERP_2].[dbo].[OT_Times_Status] AS otts1 ON ott.Start_StatusId = otts1.StatusId 
                LEFT OUTER JOIN [ERP_2].[dbo].[OT_Times_Status] AS otts2 ON ott.Stop_StatusId = otts2.StatusId
                WHERE (ott.StartTime > GETDATE() - 365) AND (ott.TrackId = o.TrackID)
            ...
            (
                ...
                FROM [ERP_2].[dbo].[OT_Times] AS ott 
                INNER JOIN [ERP_2].[dbo].[OT_Times_Employees] AS otte ON ott.TimeId = otte.TimeId 
                LEFT OUTER JOIN [ERP_2].[dbo].[v_TimeClockPlus_EmployeeList] vEmp ON otte.EmployeeId = vEmp.EmployeeId
                WHERE (ott.StartTime > GETDATE() - 365) AND (ott.TrackId = o.TrackID)
                GROUP BY ott.TimeId
                ORDER BY ott.TimeId DESC
            ) AS [Employees]
            FROM [ERP_2].[dbo].[OT] AS o 
            LEFT OUTER JOIN [ERP_2].[dbo].[OT_OrderType] AS otot ON o.OrderTypeID = otot.OrderTypeId 
...
SELECT DISTINCT 
                         ...
                         dbo.f_Branch_Form(a.Form) AS Branch, ISNULL(s.cmplt, 0) AS Completed, CASE ISNULL(s.[Hold], 0) WHEN 0 THEN (CASE shs.[SOitem] WHEN NULL THEN CONVERT(bit, 0) ELSE [ERP_2].[dbo].[f_Is_SOitemHeld](shs.[SOitem]) 
...
FROM            [ERP_2].[dbo].[PurchaseOrder] p WITH(NOLOCK) LEFT OUTER JOIN
                         [ERP_2].[dbo].[PO_NO] pn WITH(NOLOCK) ON p.HPApo = pn.HPAPO OUTER APPLY
                             (SELECT        TOP (1) [un] AS SalesPerson
                               FROM            [ERP_2].[dbo].[AppSetup] WITH(NOLOCK) 
...
FROM            [ERP_2].[dbo].[PurchaseOrder] p WITH (NOLOCK) OUTER APPLY
                             (SELECT        *
                               FROM            (SELECT        TOP (1) [ID], [Hold_Type], [Hold_Value], [Who], [What], [When], [Comment]
                                                         FROM            [ERP_2].[dbo].[Sales_Hold_Status] shs WITH (NOLOCK)
...
FROM            [ERP_2].[dbo].[Sales_Hold_Status] shs WITH (NOLOCK) JOIN
                         [ERP_2].[dbo].[PurchaseOrder] p WITH (NOLOCK) ON shs.POitem = p.POitem
WHERE 
(
