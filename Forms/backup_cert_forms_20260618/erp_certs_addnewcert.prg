* erp_certs_addnewcert.prg - extracted from erp_certs (addnewcert); was >64KB in .sct memo
LPARAMETERS toFormSet

		*toFormSet.AddNewCert()
		
		PRIVATE cHeat, cRetType
		...
		*toFormSet.ReturnLot
		...
		PRIVATE dDateCert,dDateProd
		dDateCert = Date()
		dDateProd = DATE()
		
		IF cRetType = "CLONE"
			PRIVATE cReference
			PRIVATE cHeat,nLot,cAlloy,cSize,cSpec,cCond_Fin,cUNS
		...
		*	dDateProd	= PrepareSQLdate(toFormSet.ERPCert.PF1.pgCert.txtDateProd.Value,'DateProd',.F.,.T.)
			...
			cAlloy_HPA	= ALLTRIM(PrepareSQLtxt(toFormSet.ERPCert.PF1.pgCert.Alloy1.Value,'Alloy',12))
			...
			cCond_Fin	= ALLTRIM(PrepareSQLtxt(toFormSet.ERPCert.PF1.pgCert.txtCond_Fin.Value,'Cond_Fin',50))
		...
		CASE toFormSet.ReturnWhatMach = "L" &&OR nMachine = 4
			cRetMach = "LakeErie"
			nMachine = 4
		CASE toFormSet.ReturnWhatMach = "W" &&OR nMachine = 5
			cRetMach = "WIRE"
			nMachine = 5
		CASE toFormSet.ReturnWhatMach = "S" &&OR nMachine = 6
			cRetMach = "SWAGE"
			nMachine = 6
		CASE toFormSet.ReturnWhatMach = "D" &&OR nMachine = 7
			cRetMach = "DRAW"
			nMachine = 7
		OTHERWISE
			cRetMach = "none"
		...
		*nRetLot = VAL( cRetLot )
		...
		CASE cRetType = "NEW" AND cRetMach  = "none"
			**ADD blank RECORD
			IF USED("tmpCertID")
				USE IN tmpCertID
			ENDIF
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					toFormSet.isadding=.T.
					toFormSet.SetEditing()
					toFormSet.ERPCert.PF1.pgCert.grpMach.Value = 1
					toFormSet.ERPCert.PF1.pgCert.grpMach.Refresh
					toFormSet.RefreshRefMach()
					
					MESSAGEBOX("Cert is ready",0+64,"New Cert")
				ENDIF
				USE IN tmpCertID
			ENDIF
		
		CASE cRetType = "CLONE" AND cRetMach  = "none"
			***Clone
			SELECT ERPCert
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					toFormSet.ERPCert.PF1.pgCert.Alloy1.Value = cAlloy_HPA
					
					toFormSet.ERPCert.PF1.pgCert.txtSize.Value = cSize
					toFormSet.ERPCert.PF1.pgCert.txtSpec.Value = cSpec
					toFormSet.ERPCert.PF1.pgCert.txtCond_Fin.Value = cCond_Fin
					...
					*			toFormSet.ERPCert.PF1.pgCert.txtBHN5.Value = nBHN5
					...
					MESSAGEBOX("Clone is ready",0+64,"New Cert")
				ENDIF
				USE IN tmpCertID
			ENDIF
			
		CASE cRetType = "CLONE" AND cRetMach  = "GFM"
		...
		CASE cRetType = "NEW" AND cRetMach  = "GFM"
			**ADD NEW RECORD WITH GFMLOG MECHANICALS  -no CHEMistry
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					toFormSet.SetCertNew()
					toFormSet.GetGFMlotProp(nNewLot,'')
					toFormSet.isadding=.T.
					toFormSet.SetEditing()
					toFormSet.ERPCert.PF1.pgCert.grpMach.Value = nMachine
					toFormSet.ERPCert.PF1.pgCert.grpMach.Refresh
		...
		CASE cRetType = "CLONE" AND cRetMach  = "ROLL"
		...
		CASE cRetType = "NEW" AND cRetMach  = "ROLL"
			**ADD NEW RECORD WITH ROLL_LOG MECHANICALS  -no CHEMistry
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					toFormSet.SetCertNew()
					toFormSet.GetRollLotProp(nNewLot,'')
					toFormSet.isadding=.T.
					toFormSet.SetEditing()
					toFormSet.ERPCert.PF1.pgCert.grpMach.Value = nMachine
					toFormSet.ERPCert.PF1.pgCert.grpMach.Refresh
		...
		CASE cRetType = "CLONE" AND cRetMach  = "LakeErie"
			SELECT ERPCert
			*COPY TO ARRAY aERPCert FIELDS EXCEPT ID NEXT 1
		...
		CASE cRetType = "NEW" AND cRetMach  = "LakeErie"
			...
			*OPEN LakeErie
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					toFormSet.SetCertNew()
					toFormSet.GetLakeErieLotProp(nNewLot,'')
					toFormSet.isadding=.T.
					toFormSet.SetEditing()
					toFormSet.ERPCert.PF1.pgCert.grpMach.Value = nMachine
					toFormSet.ERPCert.PF1.pgCert.grpMach.Refresh
		...
		CASE cRetType = "CLONE" AND cRetMach  = "WIRE"
			SELECT ERPCert
			*COPY TO ARRAY aERPCert FIELDS EXCEPT ID NEXT 1
		...
		CASE cRetType = "NEW" AND cRetMach  = "WIRE"
			**ADD NEW RECORD WITH LakeErie MECHANICALS  -no CHEMistry
			*OPEN LakeErie
			IF USED("tmpCertID")
				USE IN tmpCertID
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					toFormSet.SetCertNew()
					*toFormSet.GetLakeErieLotProp(nNewLot,cHeat)
					toFormSet.GetWireLotProp(nNewLot,'')
					toFormSet.isadding=.T.
					toFormSet.SetEditing()
					toFormSet.ERPCert.PF1.pgCert.grpMach.Value = nMachine
					toFormSet.ERPCert.PF1.pgCert.grpMach.Refresh
		...
		CASE cRetType = "CLONE" AND cRetMach  = "SWAGE"
			SELECT ERPCert
			*COPY TO ARRAY aERPCert FIELDS EXCEPT ID NEXT 1
			...
			SELECT 0
			cSQL = "EXEC dbo.p_Cert_ins @Machine=6"
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					*toFormSet.GetLakeErieLotProp(nNewLot,cHeat)
					toFormSet.GetSwageLotProp(nNewLot,cHeat)
					toFormSet.isadding=.T.
					toFormSet.SetEditing()
		...
		CASE cRetType = "NEW" AND cRetMach  = "SWAGE"
			**ADD NEW RECORD WITH LakeErie MECHANICALS  -no CHEMistry
			*OPEN LakeErie
			IF USED("tmpCertID")
				USE IN tmpCertID
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					toFormSet.SetCertNew()
					*toFormSet.GetLakeErieLotProp(nNewLot,cHeat)
					toFormSet.GetSwageLotProp(nNewLot,'')
					toFormSet.isadding=.T.
					toFormSet.SetEditing()
					toFormSet.ERPCert.PF1.pgCert.grpMach.Value = nMachine
					toFormSet.ERPCert.PF1.pgCert.grpMach.Refresh
		...
		*!*		ENDIF
		...
		CASE cRetType = "NEW" AND cRetMach  = "DRAW"
			**ADD NEW RECORD WITH LakeErie MECHANICALS  -no CHEMistry
			*OPEN LakeErie
			IF USED("tmpCertID")
				USE IN tmpCertID
			...
			SELECT 0
			cSQL = "EXEC dbo.p_Cert_ins @Machine=7"
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					toFormSet.SetCertNew()
					*toFormSet.GetLakeErieLotProp(nNewLot,cHeat)
					toFormSet.GetDrawLotProp(nNewLot,'')
		...
		CASE cRetType = "CLONE" AND cRetMach  = "DRAW"
			SELECT ERPCert
			*COPY TO ARRAY aERPCert FIELDS EXCEPT ID NEXT 1
			...
			IF USED('tmpCertID')
				IF VARTYPE(tmpCertID.ID) = 'N'
					...
					*toFormSet.GetLakeErieLotProp(nNewLot,cHeat)
					toFormSet.GetDrawLotProp(nNewLot,cHeat)
					toFormSet.isadding=.T.
					toFormSet.SetEditing()
					toFormSet.ERPCert.PF1.pgCert.grpMach.Value = nMachine
					toFormSet.ERPCert.PF1.pgCert.grpMach.Refresh
		...
		*toFormSet.frmAddCert.CertGroup.cmdNew.Caption = "New"
		*toFormSet.frmAddCert.CertGroup.cmdEdit.Caption = "Clone"
		
		toFormSet.RefreshERPCertCo(0)
		
		RETURN toFormSet.isadding
