  ...
  *-- Lock until user presses Esc
  ...
  IF NOT EMPTY(cAlias)
	IF USED(cAlias)
		SELECT (cAlias)
	ELSE
		RecordTest("Record Test Data.","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(),"Lost Alias '"+cAlias+"'")
...
PROCEDURE SO_get_next 
  ...
  IF NOT "PROC_SQL" $ SET("PROCEDURE")  &&Added for when Quotes is run without HPA menu.
	...
	ENDIF
	IF nSQLEXEC < 0
		MESSAGEBOX('Data Error with SQL'+CHR(13)+"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), 16, 'SQL Error')
		nSOmaxNew = 1000
	ELSE
...
IF nConnection < 0
	MESSAGEBOX("Could not connect to SQL Server"+CHR(13)+"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),16,"No Tracking")
	RETURN .F.
ENDIF

IF LEN(pTXT) > 1024
	RecordTest("Record Test Data.", "Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(),"TrackMess is too long. "+ALLTRIM(STR(LEN(pTXT)))+"."+CHR(13)+pTXT )
ENDIF
...
Pcprogram = "HPA:Proc_HPA:TrackMess"
...
IF nSQLEXEC < 0
	...
	RecordError(nSQLEXEC,"SQL Error" ,"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),LINENO(),cSQL )
ENDIF
...
*TrackError("Problem","Title","Proc_HPA"+PROGRAM(),LINENO())
...
* RecordError( nerror INT, mess1 C254,"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(1), MessMemo)
*Found in RecordError.prg and Proc_HPA
* RecordError(1, 'Title', 'program', LINENO(1),'Long Message')
* Called from TrackError of Proc_HPA
...
IF m.cMess<>'Record Test Data.'
	*Record Error of Screen
	TRY
	*	cRun = "ErrorCapture.EXE " +["Proc_HPA:RecordError: ]+SYS(0)+[" David.Kirchner@HPAlloys.com ]
	*	cRun = "ErrorCapture.EXE " +["Proc_HPA:RecordError: ]+SYS(0)+[" David.Kirchner@Windfall.HPA.local ]
		cRun = "ErrorCapture.EXE " +["Proc_HPA:RecordError: ]+SYS(0)+[" VFP@Whpalloys.com ]
...
* m.nError, m.cMess, m.cProgram, m.nLineNo, m.cMessMemo
	...
	*MESSAGEBOX(cSQL+CHR(13)+"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),0,"SQL Error")
...
* RecordTest("Record Test Data.","Proc_HPA:",1,"Long Message")
* RecordTest( mess1 C254, cprogram C254, LINENO(1), MessMemo VarChar(MAX) )
* RecordTest( "Record Test Data.","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(1),"Long Message")
...
crPOitem = ""
IF TYPE("pPOitem")="L"
	*TrackError("No POitem was sent to Proper_PO_Item (). ","Proc_HPA:Proper_PO_Item() has no Field","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),LINENO())
	RETURN '        0-00'
ENDIF
...
IF TYPE("pSOitem")="L"
	TrackError("No SOitem was sent to Proper_SO_Item (). ","Proc_HPA:Proper_SO_Item() has no Field","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),LINENO())
ENDIF
IF TYPE("pSOitem")="C"
...
IF NOT FOUND("UpsShipRoute")
	MESSAGEBOX( "UPS GROUND          "+;
	  ...
	  +CHR(13)+"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1);
...
*	ENDIF
ELSE
	TrackError("No PDF printer found! Install doPDF.","No PDF printer found","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(1),LINENO())
ENDIF
...
IF VARTYPE(nMachineNo) != "N"
	...
	*TrackError("getCertMachineInitial - nMachine number not a number","getCertMachineInitial Failed","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(1),LINENO())
	RETURN ''
ENDIF
...
IF EMPTY("cHomeSalesp")
	...
	RecordTest("Record Test Data.","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(),"No SalesP initial. Now set to: '"+cHomeSalesp+"'." )
ENDIF
...
IF EMPTY(cHomeSalesp)
	MESSAGEBOX("Bad SalesP Home!"+CHR(13)+"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),16,"EmailFileName")
	TrackError("Send Email Error","Bad SalesP Home!","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(1),LINENO())
ENDIF
IF EMPTY(cReturnFileName)
	MESSAGEBOX("File Name on Server Failed!"+CHR(13)+"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),16,"EmailFileName")
	TrackError("Send Email Error","File Name on Server Failed!"+CHR(13)+"cReturnFileName ","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(1),LINENO())
ENDIF
...
IF VARTYPE(cInput) != "C"
	...
	RecordError(0,"Encode2D Parameter error","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(), "Encode2D Parameter error")
...
cReturn = ''
TRY 
*	RecordError(0,0,"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(), "o2D = CREATEOBJECT(DataMatrix.net.BarCode)")
...
CATCH
	...
	*TrackError("Problem Memo","Title C254", PROGRAM(), LINENO(1))
	MESSAGEBOX([CREATEOBJECT("DataMatrix.net.BarCode")]+CHR(13)+"has failed!"+CHR(13)+"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),0+16,"Will not print right.")
ENDTRY
*
RELEASE o2D
******
...
IF VARTYPE(cInput) != "C"
	...
	RecordError(0,"EncodeToDataMatrixString Parameter error","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(), "EncodeToDataMatrixString Parameter error")
...
CATCH
	...
	*cmd prompt as Administrator! "C:\Windows\Microsoft.NET\Framework\v4.0.30319\RegAsm" "C:\Program Files (x86)\HPA\INCLUDE\DataMatrix.net.dll" /codebase
	...
	RecordError(0,0,"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(), "o2D = CREATEOBJECT('DataMatrix.Net.BarCode')"+CHR(13)+"DataMatrix.net.dll"+CHR(13)+[Source: "C:\Program Files (x86)\HPA\INCLUDE\DataMatrix.net.dll"; DestDir: "{sys}"; ]+CHR(13)+[Filename: "C:\Windows\Microsoft.NET\Framework\v4.0.30319\RegAsm"; Parameters: "C:\Program Files (x86)\HPA\INCLUDE\DataMatrix.net.dll /codebase";   Description: "to run DataMatrix"; Flags: postinstall shellexec skipifsilent]+CHR(13)+CHR(13)+[cmd prompt as Administrator! "C:\Windows\Microsoft.NET\Framework\v4.0.30319\RegAsm" "C:\Program Files (x86)\HPA\INCLUDE\DataMatrix.net.dll" /codebase] )
	MESSAGEBOX([CREATEOBJECT("DataMatrix.Net.BarCode")]+CHR(13)+"has failed!"+CHR(13)+"Proc_HPA:"+PROGRAM(),0+16,"Will not print right.")
ENDTRY
*
RELEASE o2D
******
...
IF VARTYPE(cInput) != "C"
	...
	RecordError(0,"EncDM Parameter error","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(), "EncDM Parameter error")
...
CATCH
	RecordError(0,0,"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(), "o2D = CREATEOBJECT(DMATRIXLib.Datamatrix)")
	MESSAGEBOX([CREATEOBJECT("DMATRIXLib.Datamatrix")]+CHR(13)+"has failed!"+CHR(13)+"Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),0+16,"Will not print right.")
ENDTRY
...
IF VARTYPE(cInput) != "C"
	...
	RecordError(0,"Convert_Fractions_to_Numeric Parameter error","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(), "Convert_Fractions_to_Numeric Parameter error")
...
IF VARTYPE(cCustID) != "C"
	RecordError( 0, 'PARAMETERS ', "Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1), LINENO(),"VFP message: No cCustID Parameter" )
	RETURN .F.
ENDIF
...
ELSE
	*Error on Connection String
	TrackError("Could not Connect to SQL","Error on Connection String","Proc_HPA:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),LINENO())
ENDIF
