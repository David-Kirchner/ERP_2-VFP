...
IF cUnit = ''
	*Check History
	...
	DO CASE
	CASE cTBL = "S"
		cSQL = cSQL + " dbo.Ar_Stocklst_Master_History "
	CASE cTBL = "B"
		cSQL = cSQL + " dbo.Ar_BrokerLst_Master_history "
	CASE cTBL = "W"
		cSQL = cSQL + " dbo.Ar_WIPLst_Master_History "
	ENDCASE
	cSQL = cSQL + " WHERE ID="+ALLTRIM(STR(nID))
	cSQL = cSQL + " AND LEN(Form)>1"
...
DO CASE
CASE cTBL = "S"
	cSQL = cSQL + " dbo.Ar_Stocklst_Detail_History "
CASE cTBL = "B"
	cSQL = cSQL + " dbo.Ar_BrokerLst_Detail_History "
CASE cTBL = "W"
	cSQL = cSQL + " dbo.Ar_WIPlst_Detail_History "
OTHERWISE 
	TrackError("Bad Parameter nConn","Bad Parameter","Proc_StockLst:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),LINENO())	
	RETURN 0
ENDCASE
...
CASE cTBL = "S"
	...
	cSQL = cSQL + "UNION ALL "
	cSQL = cSQL + "SELECT PO_Item "
	cSQL = cSQL + "	FROM dbo.Ar_Stocklst_Master_History "
	cSQL = cSQL + "	WHERE ID="+ALLTRIM(STR( pnID ))
	cSQL = cSQL + "		 AND LEN(PO_Item)>1"
	cSQL = cSQL + " )"
...
CASE cTBL = "B"
	...
	cSQL = cSQL + "UNION ALL "
	cSQL = cSQL + "SELECT PO_Item "
	cSQL = cSQL + "	FROM dbo.Ar_BrokerLst_Master_history "
	cSQL = cSQL + "	WHERE ID="+ALLTRIM(STR( pnID ))
	cSQL = cSQL + "		 AND LEN(PO_Item)>1"
	cSQL = cSQL + " )"
	cSQL = cSQL + " AS tmpPSTK_ "
...
CASE cTBL = "W"
	...
	cSQL = cSQL + "UNION ALL "
	cSQL = cSQL + "SELECT PO_Item "
	cSQL = cSQL + "	FROM dbo.Ar_WIPLst_Master_History "
	cSQL = cSQL + "	WHERE ID="+ALLTRIM(STR( pnID ))
	cSQL = cSQL + "		 AND LEN(PO_Item)>1"
	cSQL = cSQL + " )"
	cSQL = cSQL + " AS tmpPSTK_ "
...
IF nConn > 0
	...
	ELSE
		*history	
		cSQL = cSQL + " UNION "
		cSQL = cSQL + " SELECT TOP 1 'SH' AS TBL FROM dbo.Ar_Stocklst_Detail_History "
		cSQL = cSQL + " WHERE ID = "+STR(nID)+" AND ID_Detail ="+STR(nID_Detail)+" AND [What] = 'DELETE'"
		cSQL = cSQL + " UNION "
		cSQL = cSQL + " SELECT TOP 1 'BH' AS TBL FROM dbo.Ar_BrokerLst_Detail_History "
		cSQL = cSQL + " WHERE ID = "+STR(nID)+" AND ID_Detail ="+STR(nID_Detail)+" AND [What] = 'DELETE'"
		cSQL = cSQL + " UNION "
		cSQL = cSQL + " SELECT TOP 1 'WH' AS TBL FROM dbo.Ar_WIPlst_Detail_History "
		cSQL = cSQL + " WHERE ID = "+STR(nID)+" AND ID_Detail ="+STR(nID_Detail)+" AND [What] = 'DELETE'"
...
IF nConn > 0
	...
	cSQL = cSQL + " FROM dbo.Ar_Stocklst_Detail_History "
	cSQL = cSQL + " WHERE ID_Detail = "+STR(nID_Detail)
...
CASE cTBL == "BH"
	cSQL = cSQL+" FROM dbo.Ar_BrokerLst_Detail_History "
	cSQL = cSQL+" WITH(NOLOCK) " &&could be in transaction lock
	cSQL = cSQL+" WHERE ID = "+ STR(nID)
	cSQL = cSQL+" AND [What]='DELETE'"
CASE cTBL == "WH"
	cSQL = cSQL+" FROM dbo.Ar_WIPlst_Detail_History "
	cSQL = cSQL+" WITH(NOLOCK) " &&could be in transaction lock
	cSQL = cSQL+" WHERE ID = "+ STR(nID)
	cSQL = cSQL+" AND [What]='DELETE'"
ENDCASE
...
CASE cTBL == "SH"
	cSQL = cSQL+" FROM dbo.Ar_StockLst_Process_History "
	cSQL = cSQL+" WITH(NOLOCK) " &&could be in transaction lock
	cSQL = cSQL+" WHERE ID_Detail = "+ STR(nID_Detail)
	cSQL = cSQL+" AND [What]='DELETE'"
CASE cTBL == "BH"
	cSQL = cSQL+" FROM dbo.Ar_BrokerLst_Process_History "
	cSQL = cSQL+" WITH(NOLOCK) " &&could be in transaction lock
	cSQL = cSQL+" WHERE ID_Detail = "+ STR(nID_Detail)
	cSQL = cSQL+" AND [What]='DELETE'"
CASE cTBL == "WH"
	cSQL = cSQL+" FROM dbo.Ar_WIPLst_Process_History "
	cSQL = cSQL+" WITH(NOLOCK) " &&could be in transaction lock
	cSQL = cSQL+" WHERE ID_Detail = "+ STR(nID_Detail)
	cSQL = cSQL+" AND [What]='DELETE'"
ENDCASE
...
*!*					
*!*	*!*					IF pFrom = "Edit PO"
*!*	*!*						cSQL_Delete="DELETE FROM dbo.Ar_Stocklst_Detail_History WHERE ID_Detail = "+STR(pID_Detail)
*!*	*!*						nSQLEXEC = SQLEXEC(pConnHandle, cSQL_Delete )
*!*	*!*						DO WHILE nSQLEXEC = 0
*!*	*!*							WAIT WINDOW 'SQL' TIMEOUT 1
*!*	*!*							nSQLEXEC = SQLEXEC(pConnHandle, cSQL_Delete )
...
*!*					
*!*	*!*					IF pFrom = "Edit PO"
*!*	*!*						cSQL_Delete="DELETE FROM dbo.Ar_BrokerLst_Detail_History WHERE ID_Detail = "+STR(pID_Detail)
*!*	*!*						nSQLEXEC = SQLEXEC(pConnHandle, cSQL_Delete )
*!*	*!*						DO WHILE nSQLEXEC = 0
*!*	*!*							WAIT WINDOW 'SQL' TIMEOUT 1
*!*	*!*							nSQLEXEC = SQLEXEC(pConnHandle, cSQL_Delete )
...
*!*					
*!*	*!*					IF pFrom = "Edit PO"
*!*	*!*						cSQL_Delete="DELETE FROM dbo.Ar_WIPlst_Detail_History WHERE ID_Detail = "+STR(pID_Detail)
*!*	*!*						nSQLEXEC = SQLEXEC(pConnHandle, cSQL_Delete )
*!*	*!*						DO WHILE nSQLEXEC = 0
*!*	*!*							WAIT WINDOW 'SQL' TIMEOUT 1
*!*	*!*							nSQLEXEC = SQLEXEC(pConnHandle, cSQL_Delete )
...
CASE cTBL = "S"
	...
	cSQL = cSQL+" FROM dbo.Ar_StockLst_Process_History P "
	cSQL = cSQL+" INNER JOIN dbo.Ar_Stocklst_Detail_History D ON P.ID_Detail = D.ID_Detail "
...
CASE cTBL = "B"
	cSQL = "SELECT TOP 1 "
	cSQL = cSQL+" P.ID_Detail_Process "
	cSQL = cSQL+" FROM dbo.Ar_BrokerLst_Process_History P "
	cSQL = cSQL+" INNER JOIN dbo.Ar_BrokerLst_Detail_History D ON P.ID_Detail = D.ID_Detail "
	cSQL = cSQL+" WHERE P.Process_ID='INCOMING  '"
	cSQL = cSQL+" AND P.[what] = 'ADD' "
	cSQL = cSQL+" AND D.ReceivingID = "+ALLTRIM(STR( pnReceiveID ))
...
CASE cTBL = "W"
	cSQL = "SELECT TOP 1 "
	cSQL = cSQL+" P.ID_Detail_Process "
	cSQL = cSQL+" FROM dbo.Ar_WIPLst_Process_History P "
	cSQL = cSQL+" INNER JOIN dbo.Ar_WIPlst_Detail_History D ON P.ID_Detail = D.ID_Detail "
	cSQL = cSQL+" WHERE P.Process_ID='INCOMING  '"
	cSQL = cSQL+" AND P.[what] = 'ADD' "
	cSQL = cSQL+" AND D.ReceivingID = "+ALLTRIM(STR( pnReceiveID ))
...
cSQL =        " SELECT 'S' AS TBL FROM dbo.Ar_StockLst_Process_History WHERE ReceivingID = "+STR(nReceivingID)
cSQL = cSQL + " UNION "
cSQL = cSQL + " SELECT 'B' AS TBL FROM dbo.Ar_BrokerLst_Process_History WHERE ReceivingID = "+STR(nReceivingID)
cSQL = cSQL + " UNION "
cSQL = cSQL + " SELECT 'W' AS TBL FROM dbo.Ar_WIPLst_Process_History WHERE ReceivingID = "+STR(nReceivingID)
...
cSQL = cSQL + " FROM dbo.Ar_StockLst_Process_History P "
cSQL = cSQL + " INNER JOIN dbo.Ar_Stocklst_Detail_History D ON P.ID_Detail = D.ID_Detail "
cSQL = cSQL + " INNER JOIN dbo.Ar_Stocklst_Master_History M ON D.ID = M.ID "

CASE cTBL = "B"
cSQL = cSQL + " SELECT DISTINCT 'B' AS TBL, P.ID_Detail, P.ID_Detail_Process "
cSQL = cSQL + " FROM dbo.Ar_BrokerLst_Process_History P "
cSQL = cSQL + " INNER JOIN dbo.Ar_BrokerLst_Detail_History D ON P.ID_Detail = D.ID_Detail "
cSQL = cSQL + " INNER JOIN dbo.Ar_BrokerLst_Master_history M ON D.ID = M.ID "

CASE cTBL = "W"
cSQL = cSQL + " SELECT DISTINCT 'W' AS TBL, P.ID_Detail, P.ID_Detail_Process "
cSQL = cSQL + " FROM dbo.Ar_WIPLst_Process_History P "
cSQL = cSQL + " INNER JOIN dbo.Ar_WIPlst_Detail_History D ON P.ID_Detail = D.ID_Detail "
cSQL = cSQL + " INNER JOIN dbo.Ar_WIPLst_Master_History M ON D.ID = M.ID "
ENDCASE
...
DO CASE
CASE cTBL = "S"
cSQL = cSQL + " SELECT 'S' AS TBL, ID_Detail FROM dbo.Ar_StockLst_Process_History "
CASE cTBL = "B"
cSQL = cSQL + " SELECT 'B' AS TBL, ID_Detail FROM dbo.Ar_BrokerLst_Process_History "
CASE cTBL = "W"
cSQL = cSQL + " SELECT 'W' AS TBL, ID_Detail FROM dbo.Ar_WIPLst_Process_History "
ENDCASE
...
DO CASE
CASE cTBL = "S"
	cSQL = cSQL + " dbo.Ar_Stocklst_Detail_History WITH(NOLOCK) "
CASE cTBL = "B"
	cSQL = cSQL + " dbo.Ar_BrokerLst_Detail_History WITH(NOLOCK) "
CASE cTBL = "W"
	cSQL = cSQL + " dbo.Ar_WIPlst_Detail_History WITH(NOLOCK) "
OTHERWISE 
...
IF nConn > 0
	*Look for the 'DELETE' in history
	...
	DO CASE
	CASE cTBL = "S"
		cSQL=cSQL+" FROM dbo.Ar_Stocklst_Master_History WITH(NOLOCK) "
	CASE cTBL = "B"
		cSQL=cSQL+" FROM dbo.Ar_BrokerLst_Master_history WITH(NOLOCK) "
	CASE cTBL = "W"
		cSQL=cSQL+" FROM dbo.Ar_WIPLst_Master_History WITH(NOLOCK) "
	OTHERWISE 
		TrackError("Bad Parameter","Bad Parameter nConn","Proc_StockLst:"+PROGRAM()+" @"+PROGRAM(PROGRAM(-1)-1),LINENO())	
		RETURN .F.
	ENDCASE
	...
	****************
	IF lOK AND nID_History > 0
		...
		*Fill in ALL fields now
		...
		DO CASE 
		CASE cTBL = "S"
			cSQL = cSQL + " FROM dbo.Ar_Stocklst_Master_History SM4 "
			cSQL = cSQL + " WHERE StockLst_Master.ID = "+STR(nID)
		CASE cTBL = "B"
			cSQL = cSQL + " FROM dbo.Ar_BrokerLst_Master_history SM4 "
			cSQL = cSQL + " WHERE BrokerLst_Master.ID = "+STR(nID)
		CASE cTBL = "B"
			cSQL = cSQL + " FROM dbo.Ar_WIPLst_Master_History SM4 "
			cSQL = cSQL + " WHERE BrokerLst_Master.ID = "+STR(nID)
		ENDCASE	
